...
def init_brt_hjivi_TV_loss(dynamics, minWith, dirichlet_loss_divisor, MPC_loss_type, use_MPC, MPC_finetune_lambda):
    def brt_hjivi_TV_loss(state, value, dvdt, dvds, boundary_value, dirichlet_mask, output, MPC_values, MPC_labels,
                        use_MPC_terminal_loss=False, t=None, T=None):
        # time-varying obstacle's control
        # Curriculum training loss
        if dynamics.deepReach_model == 'exact':
            dirichlet_loss = torch.Tensor([0]).cuda()
        else:
        ...
        if torch.all(dirichlet_mask): # pretraining loss
            ...
            ham = dynamics.hamiltonian(state, dvds, t ,T)
            if minWith == 'zero':
                ham = torch.clamp(ham, max=0.0)
