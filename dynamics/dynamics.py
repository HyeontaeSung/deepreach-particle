...
class TwoDubins3D_TimeVarying(Dynamics):
    ...
    def __init__(self, set_mode: str):
        ...
        state_ranges = [
            [-1.0, 1.0],
            [-1.0, 1.0],
            [-math.pi, math.pi],  # First vehicle
            [-1.0, 1.0],
            [-1.0, 1.0],
            [-math.pi, math.pi],  # Second vehicle
        ]
        # Add control sequence ranges
        for _ in range(self.control_sequence_length):
            state_ranges.append([-self.omega_max, self.omega_max])
        ...
        state_var_ = (self.state_range_[:, 1] - self.state_range_[:, 0]) / 2.0
        super().__init__(
            name="TwoDubins3D_TimeVarying",
            loss_type=l_type,
            set_mode=set_mode,
            state_dim=6 + self.control_sequence_length,
            input_dim=9 + self.control_sequence_length,
            control_dim=1,
            disturbance_dim=0,
            state_mean=state_mean_.cpu().tolist(),
            state_var=state_var_.cpu().tolist(),
    ...
    def interpolate_control(self, state, time, T):
        ...
        if isinstance(time, (int, float)):
            time = torch.tensor(
                self.tMax - T + time, device=state.device, dtype=state.dtype
            )  # for MPC
        else:
            time = self.tMax - time.to(device=state.device)  # for DeepReach
    ...
    def constant_interpolate_control(self, state, time, T):
        ...
        control_value = torch.gather(control_sequence, -1, index.unsqueeze(-1)).squeeze(
            -1
        )
    ...
    def avoid_fn(self, state):
        ...
        for obstacle in self.obstacles:
            ...
            avoid_val = torch.minimum(
                avoid_val, (torch.norm(rel_state, dim=-1) - obstacle[2])
            )
    ...
    def plot_config(self):
        return {
            ...
            "state_labels": ["x1", "y1", r"$\theta$1", "x2", "y2", r"$\theta$2", "u1", "u2"]
            + [f"u_{i}" for i in range(self.control_sequence_length)],
            "x_axis_idx": 0,
            "y_axis_idx": 1,
            "z_axis_idx": 6,
        }
...
class TwoDubins3D_Worst(Dynamics):
    ...
    def __init__(self, set_mode: str):
        ...
        self.control_range_ = torch.tensor(
            [[-self.omega_max, self.omega_max], [-self.omega_max, self.omega_max]]
        ).cuda()
        self.eps_var = torch.tensor([1]).cuda()
        ...
        if self.set_mode == "avoid":
            h = (
                ...
                * (
                ...
                + self.velocity
                * (
                    torch.cos(state[..., 5]) * dvds[..., 3]
                    + torch.sin(state[..., 5]) * dvds[..., 4]
                )
                - self.omega_max * torch.abs(dvds[..., 5])
            )
            return h
        elif self.set_mode in ["reach", "reach_avoid"]:
            return self.velocity * (
...
class Dubins3D2D(Dynamics):
    def __init__(self, set_mode: str):
        ...
        self.control_range_ = torch.tensor(
            [[-self.omega_max, self.omega_max], [self.v_min, self.v_max]]
        ).cuda()
        self.eps_var = torch.tensor([1]).cuda()
        self.control_init = torch.zeros(2).cuda()
        self.set_mode = set_mode
    ...
    def optimal_control(self, state, dvds):
        if self.set_mode == "avoid":
            ...
            # Use element-wise operations instead of conditional
            optimal_v = torch.where(optimal_v_indicator >= 0, self.v_max, self.v_min)
...
class Dubins3D2D_Human_Worst(Dynamics):
    def __init__(self, set_mode: str):
        ...
        self.state_range_ = torch.tensor(
            [[-5, 5], [-5, 5], [-math.pi, math.pi], [-5, 5], [-5, 5]]
        ).cuda()
        self.control_range_ = torch.tensor(
            [
                ...
                [-math.pi, math.pi],
                [self.v_human_min, self.v_human_max],
            ]
        ).cuda()
        self.eps_var = torch.tensor([1]).cuda()
        self.control_init = torch.zeros(2).cuda()
    ...
    def optimal_control(self, state, dvds):
        if self.set_mode == "avoid":
            ...
            optimal_heading_human = torch.atan2(
                -dvds[..., 4], -dvds[..., 3]
            )  # return in [-pi, pi]
            optimal_v_human = torch.full_like(optimal_omega, self.v_human_max)
            return torch.stack(
                [optimal_omega, optimal_v, optimal_heading_human, optimal_v_human],
                dim=-1,
            )
        elif self.set_mode == "reach":
            return -(self.omega_max * torch.sign(dvds[..., 2]))[..., None]
...
class Dubins3D2D_Human_predict(Dynamics):
    def __init__(self, set_mode: str):
        ...
        self.state_range_ = torch.tensor(
            [
                ...
                [-5, 5],
                [-math.pi, math.pi],
                [self.v_human_min, self.v_human_max * 1.1],
            ]
        ).cuda()
        self.control_range_ = torch.tensor(
            [[-self.omega_max, self.omega_max], [self.v_min, self.v_max]]
    ...
    def hamiltonian(self, state, dvds):
        if self.set_mode == "avoid":
            ...
            h = (
                h
                + dvds[..., 3] * state[..., 6] * torch.cos(state[..., 5])
                + dvds[..., 4] * state[..., 6] * torch.sin(state[..., 5])
            )
            return h
...
class Dubins3D2D_Human_predict_TV(Dynamics):
    def __init__(self, set_mode: str):
        ...
        self.eps_var = torch.tensor([1.0, 1.0]).cuda()
        self.control_init = torch.zeros(2).cuda()
        self.set_mode = set_mode
        ...
        state_var_ = (self.state_range_[:, 1] - self.state_range_[:, 0]) / 2.0
        super().__init__(
            name="Dubins3D2D_Human_predict_TV",
            loss_type="brt_hjivi_TV",
            set_mode=set_mode,
            state_dim=9,
            input_dim=13,
            control_dim=2,
    ...
    def equivalent_wrapped_state(self, state):
        ...
        wrapped_state[..., 5:7] = (wrapped_state[..., 5:7] + math.pi) % (
            2 * math.pi
        ) - math.pi
        return wrapped_state
    ...
    def periodic_transform_fn(self, input):
        ...
        transformed_input = torch.zeros(output_shape).cuda()
        transformed_input[..., :3] = input[..., :3]
        transformed_input[..., 3] = torch.sin(input[..., 3] * self.state_var[2])
        transformed_input[..., 4] = torch.cos(input[..., 3] * self.state_var[2])
        transformed_input[..., 5:7] = input[..., 4:6]
        transformed_input[..., 7] = torch.sin(input[..., 6] * self.state_var[5])
        transformed_input[..., 8] = torch.cos(input[..., 6] * self.state_var[5])
        transformed_input[..., 9] = torch.sin(input[..., 7] * self.state_var[6])
        transformed_input[..., 10] = torch.cos(input[..., 7] * self.state_var[6])
        transformed_input[..., 11:] = input[..., 8:]

        return transformed_input.requires_grad_(True)
    ...
    def hamiltonian(self, state, dvds, time=None, T=None):
        if self.set_mode == "avoid":
            ...
            h = (
                ...
                + dvds[..., 3]
                * control_human[..., 1]
                * torch.cos(control_human[..., 0])
                + dvds[..., 4]
                * control_human[..., 1]
                * torch.sin(control_human[..., 0])
            )
            return h
...
class Dubins4D_Human_Predict_TV(Dynamics):
    ...
    def __init__(self, set_mode: str):
        ...
        self.control_range_ = torch.tensor(
            [[-self.omega_max, self.omega_max], [self.a_min, self.a_max]]
        ).cuda()
        self.eps_var = torch.tensor([1.0, 2.0]).cuda()
        self.control_init = torch.zeros(2).cuda()
        self.set_mode = set_mode
        ...
        state_var_ = (self.state_range_[:, 1] - self.state_range_[:, 0]) / 2.0
        super().__init__(
            name="Dubins4D_Human_Predict_TV",
            loss_type="brt_hjivi_TV",
            set_mode=set_mode,
            state_dim=10,
            input_dim=14,
    ...
    def control_range(self, state):
        return [[-self.omega_max, self.omega_max], [self.a_min, self.a_max]]
    ...
    def equivalent_wrapped_state(self, state):
        ...
        wrapped_state[..., 3] = torch.clamp(
            wrapped_state[..., 3], min=self.v_min, max=self.v_max
        )  # bound the velocity
        wrapped_state[..., 6:8] = (wrapped_state[..., 6:8] + math.pi) % (
            2 * math.pi
        ) - math.pi
        return wrapped_state
    ...
    def clamp_control(self, state, control):
        ...
        vmax_mask = state[..., 3] > self.state_range_[3, 1] - 0.001
        vmin_mask = state[..., 3] < self.state_range_[3, 0] + 0.001
        if len(vmax_mask.shape) == 1:
            max_sample_idx = vmax_mask.nonzero(as_tuple=True)[0]
            control_clamped[max_sample_idx, 1] = torch.clamp(
                control_clamped[max_sample_idx, 1], max=0
            )
            min_sample_idx = vmin_mask.nonzero(as_tuple=True)[0]
            control_clamped[min_sample_idx, 1] = torch.clamp(
                control_clamped[min_sample_idx, 1], min=0
            )
        else:
            max_batch_idx, max_sample_idx = vmax_mask.nonzero(as_tuple=True)
            control_clamped[max_batch_idx, max_sample_idx, 1] = torch.clamp(
                control_clamped[max_batch_idx, max_sample_idx, 1], max=0
            )
            min_batch_idx, min_sample_idx = vmin_mask.nonzero(as_tuple=True)
            control_clamped[min_batch_idx, min_sample_idx, 1] = torch.clamp(
                control_clamped[min_batch_idx, min_sample_idx, 1], min=0
            )
    ...
    def boundary_fn(self, state):
        return torch.norm(state[..., :2] - state[..., 4:6], dim=-1) - self.goalR
    ...
    def optimal_control(self, state, dvds):
        if self.set_mode == "avoid":
            ...
            #     self.a_max
            #     * torch.sign(dvds[..., 3])
            #     * ((state[..., 3] > self.v_min) & (state[..., 3] < self.v_max))
            # )
            acc_optimal = self.a_max * torch.sign(dvds[..., 3])
    ...
    def plot_config(self):
        return {
            "state_slices": [0, 0, 0, self.v_max, 0, 0, 3.14, 3.14, 1, 1],
            "state_labels": [
                "x",
                "y",
                r"$\theta$",
...
class Dubins4D_Human_Worst(Dynamics):
    """
    Dubins4D + human obstacle with worst-case control
    """
    ...
    def __init__(self, set_mode: str):
        ...
        self.eps_var = torch.tensor([1.0]).cuda()
        self.control_init = torch.tensor([0.0, 0.0, 0.0, (self.v_human_max+self.v_human_min)/2.0]).cuda()
        self.set_mode = set_mode

        state_mean_ = (self.state_range_[:, 0] + self.state_range_[:, 1]) / 2.0
        state_var_ = (self.state_range_[:, 1] - self.state_range_[:, 0]) / 2.0
        super().__init__(
            name="Dubins4D_Human_Worst",
            loss_type="brt_hjivi",
            set_mode=set_mode,
            state_dim=6,
            input_dim=8,
    ...
    def dsdt(self, state, control, disturbance, time=None, T=None):
        ...
        dsdt[..., 2] = control[..., 0]
        dsdt[..., 3] = control[..., 1]
        dsdt[..., 4] = control[..., 3] * torch.cos(control[..., 2])
        dsdt[..., 5] = control[..., 3] * torch.sin(control[..., 2])
    ...
    def optimal_control(self, state, dvds):
        if self.set_mode == "avoid":
            ...
            # )
            acc_optimal = self.a_max * torch.sign(dvds[..., 3])
            optimal_heading_human = torch.atan2(-dvds[..., 5], -dvds[..., 4]) # return in [-pi, pi]
            optimal_v_human = torch.full_like(optimal_heading_human, self.v_human_max)
            return torch.stack([omega_optimal, acc_optimal, optimal_heading_human, optimal_v_human], dim=-1)
        elif self.set_mode == "reach":
            return -(self.omega_max * torch.sign(dvds[..., 2]))[..., None]
        else:
            raise NotImplementedError
    ...
    def plot_config(self):
        return {
            "state_slices": [0, 0, 0, self.v_max, 0, 0],
            "state_labels": [
                "x",
                "y",
                r"$\theta$",
